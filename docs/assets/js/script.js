...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const openModalBtn = document.querySelector(".button.primary"); // Adjusted to class for Availability button
    const closeModalBtn = document.getElementById("close-modal"); // Matches the "X" button ID in index.html
    ...
    document.getElementById("add-slot").addEventListener("click", () => {
        ...
        if (timeSlotsContainer.childElementCount < 3) {
            const slotNumber = timeSlotsContainer.childElementCount + 1;
    ...
    document.getElementById("availability-form").addEventListener("submit", async (event) => {
        ...
        const timeSlotsData = Array.from(document.querySelectorAll(".time-slot")).map(slot => ({
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                await updateDoc(doc(db, "availability", userDoc.id), { [updateField]: timeSlotsContainer });
                alert(`Availability updated for existing user (${selectedType}).`);
            } else {
                await addDoc(usersCollection, {
                    name: nameInput,
                    weekdayTimeSlots: selectedType === "weekday" ? timeSlotsContainer : [],
                    weekendTimeSlots: selectedType === "weekend" ? timeSlotsContainer : []
                });
    ...
    async function loadHeatmap() {
        ...
        snapshot.forEach(doc => {
            const timeSlotsContainer = doc.data()[selectedType === "weekday" ? "weekdayTimeSlots" : "weekendTimeSlots"] || [];
